#!/usr/bin/env python3
"""Harvest ALL SEG Technical Program Expanded Abstracts from Crossref.

OpenAlex's coverage of SEG conference proceedings is sparse (~368
entries). Crossref has them all under DOI prefix `10.1190/`. This
script pulls every `type:proceedings-article` from that prefix,
year by year, and writes a JSONL matching `harvest.py`'s schema so
`jsonl_to_bib.py` can consume it directly.

Usage
-----
    python scripts/harvest_seg.py
    # or in tsp:
    tsp -L seg-harvest python scripts/harvest_seg.py

Output
------
    ~/Desktop/seisbib_harvest/seg_papers.jsonl       JSONL (one paper/line)
    ~/Desktop/seisbib_harvest/seg_seen_dois.txt      dedup
    ~/Desktop/seisbib_harvest/seg_state.json         resume state

Integration
-----------
    python scripts/jsonl_to_bib.py \
        --jsonl ~/Desktop/seisbib_harvest/seg_papers.jsonl \
        --since 2015 --limit 1000 --sort-by-cites \
        --out /tmp/cand_seg.bib
"""
...
PAPERS_FILE = OUTDIR / "seg_papers.jsonl"
SEEN_FILE = OUTDIR / "seg_seen_dois.txt"
STATE_FILE = OUTDIR / "seg_state.json"

USER_EMAIL = "shaowen.wang@kaust.edu.sa"

# SEG annual meeting goes back to 1931; expanded-abstracts era starts ~1981.
# Crossref DOI prefix 10.1190 belongs to SEG.
YEAR_RANGE = list(range(1981, datetime.now(timezone.utc).year + 1))
...
def fetch_page(year: int, cursor: str) -> dict:
    params = {
        "filter": (
            f"prefix:10.1190,type:proceedings-article,"
    ...
    req = urllib.request.Request(
        url,
        headers={"User-Agent": f"seisbib-segharvest/1.0 (mailto:{USER_EMAIL})"},
    )
...
def to_record(work: dict, year: int) -> dict:
    ...
    return {
        ...
        "venue": venue or "SEG Technical Program Expanded Abstracts",
        ...
        "openalex_id": None,
        "discovered_via_query": f"crossref/seg/{year}",
